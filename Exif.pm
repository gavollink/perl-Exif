...
# Exif.pm
#
# Author: Gary Allen Vollink, (c) 2017, 2022
#
####
#
...
sub init
{
    ...
    if ( 0 == scalar(@args) ) {
        ...
        if ( q{HASH} eq ref( $args[0] ) ) {
            ...
            return;
        } ## end else [ if ( q{HASH} eq ref( $args...))]

    } ## end elsif ( 1 == scalar(@args...))
...
sub filename
{
    ...
    my $sixtyFourK = q{};
    read( $fh, $sixtyFourK, 64536, 0 )
        || critical("$name: Unable to read: $!");
    close($fh);
...
sub readExifEnvelope
{
    ...
    TP: for ( my $cx = 0; $cx < $#{$buff}; $cx++ ) {
        if ( !defined $endian ) {
            ...
            if ( 8 > $offset ) {
                my $err
                    = sprintf( qq{%s: Tiff Header Offset too small (%s %d)},
                        $name, $endian, $offset, );
                critical($err);
            } ## end if ( 8 > $offset )
            ...
            if ( !exists( $self->{'Exif_IFD'} ) ) {
                ...
                $self->{'Exif_IFD'}->{ $self->{'Exif_cnt'} }
                    = { 'comment' => $cur_title, };
                if ( $cur_title =~ m{^GPS} ) {
                    $self->{'Exif_IFD'}->{ $self->{'Exif_cnt'} }->{'Type'}
                        = q{GPS};
                }
                elsif ($cur_title =~ m{^MakerNote}
                    && $self->{'Make'} eq q{Canon} )
                {
                    $self->{'Exif_IFD'}->{ $self->{'Exif_cnt'} }->{'Type'}
                        = q{MakerCanon};
                } ## end elsif ( $cur_title =~ m{^MakerNote}...)
            } ## end else [ if ( !exists( $self->{...}))]
            if ( $cx == $self->{'Exif_offset'}->[ $self->{'Exif_cnt'} ] ) {
                    ...
                    && ( $self->{'MakerNote_offset'} ) )
                {
                    ...
                    if ( $self->{'Make'} eq 'NEVER' ) {
                        ...
                        $cx = $#{$buff};
                        next TP;
                    } ## end else [ if ( $self->{'Make'} eq...)]
                } ## end elsif ( ( $self->{'MakerNote_size'...}))
                else {
                    $cx = $#{$buff};
                    next TP;
                }
            } ## end if ( $cx == $self->{'Exif_offset'...})
            else {
            ...
            $cx--;                      # For incrementor
            next TP;
        } ## end else [ if ( !defined $endian )]
    } ## end TP: for ( my $cx = 0; $cx <...)
} ## end sub readExifEnvelope
...
sub readIFD
{
    ...
    } ## end foreach my $ifd_cx ( 0 .. $ifd_cnt...)
    vdebug(
        sprintf(
            "%012d: Next IFD Offset:\n%s",
...
sub readIFDTag
{
    ...
    debug(
        sprintf(
            "TAG 0x%04x: FMT 0x%04x: NOC 0x%08x: ADR 0x%08x (%d)",
            $tag, $fmt, $noc, $adr, $adr
        )
    );
    ...
    if ( q{MakerCanon} eq $type ) {
        $ifd->{'record'}->[$ifd_cx]->{'tag_name'}
            = _canonMakerNoteTagName($tag);
    }
    elsif ( q{GPS} eq $type ) {
    ...
    if ( 0x0112 == $tag && 3 == $fmt ) {
    ...
    } ## end elsif ( 0x9209 == $tag )
    elsif ( 0x920a == $tag ) {
        ####################################
        # FocalLength
        $str = qq{$str mm};
        $ifd->{'record'}->[$ifd_cx]->{'val_exp'} = $str;
    } ## end elsif ( 0x920a == $tag )
        ...
        $ifd->{'record'}->[$ifd_cx]->{'val_exp'} = $str;
    } ## end elsif ( 0xa210 == $tag &&...)

    debug(
        sprintf(
            "TAG %s: FMT %s: NOC %d: ADR %s",
...
#            }
        } ## end if ( q{HASH} eq ref $opt)
        else {
            critical(  "$name: Option passed as " . ref($opt)
                    || 'string' . "." );
        }
    } ## end if ( defined $opt )
#    else {
...
#    }
    ...
    for ( my $cx = 0; $cx < $ifd_cx; $cx++ ) {
        ...
        if ( !defined $records ) {
            ...
            critical(
                "$name: Tags not formatted correctly (expecting array).");
        }

        if ( $count != scalar(@$records) ) {
            my $tmp = scalar(@$records);
            critical(
                "$name: Tag array count incorrect (got $tmp expecting $count)."
            );
        } ## end if ( $count != scalar(...))
        ...
        foreach my $rec ( @{$records} ) {
            ...
            elsif ( exists $rec->{'val'}->{'out'}
                && defined( $rec->{'val'}->{'out'} ) )
            {
                $val = $rec->{'val'}->{'out'};
            }
            elsif ( exists $rec->{'val'}->{'num'}
                && defined( $rec->{'val'}->{'num'} ) )
            {
                $val = $rec->{'val'}->{'num'};
            }
...
sub _readOrientation
{
    ...
    if ( 0xFF000000 & $val ) {
        ...
        $val = ( $val >> 16 );
    } ## end elsif ( 0x00FF0000 & $val)
    return $val;
} ## end sub _readOrientation
...
sub _strInBuff
{
    ...
    if ( !defined $address ) {
        ...
        critical(
            "_strInBuff: Address, $address+$noc, is beyond buffer size, $max."
        );
    }
...
sub _hexDump
{
    ...
    if ( $max < $offset ) {
        confess(
            "Request to _hexDump outside of loaded buffer ($max < $offset).");
    }
    if ( $max < $offset + $noc ) {
        debug(
            "Request to _hexDump past loaded buffer ($max < $offset), loading what exists."
        );
        $noc         = $max - $offset;
        $end_of_data = q{<EOB>};
    } ## end if ( $max < $offset + ...)
...
sub _bytesToInt
{
    ...
    if ( $offset + $length > $max ) {
        critical(
            qq{_bytesToInt :: offset+length on buffer is too large ($offset+$length).}
        );
    }
...
sub _expValueFromStream
{
    ...
    if ( 1 == $fmt ) {
        ...
        } ## end foreach ( my $cx = 0; $cx <...)
    } ## end if ( 1 == $fmt )
    elsif ( 2 == $fmt ) {
        #return q{ascii strings};
        if ( 4 >= $noc ) {
            $val->{'byte'}
                = [ @{$buff}[ $v_offset .. $v_offset + ( $noc - 1 ) ] ];
...
sub _float8byte
{
    ...
    #### Create Exponent
    my $doexpo = Math::BigInt->new($new)->band($expomask)->bdiv($denomina)
        ->bsub(1023);
...
} ## end sub _canonMakerNoteTagName
